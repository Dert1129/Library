import { StyleSheet, View, StatusBar, FlatList, TouchableOpacity, Image, Text, TextInput } from 'react-native';
...
export const TrackingScreen: React.FC<HomeScreenProps> = ({ route, navigation }) => {
  ...
  const getBooks = useCallback(() => {
    axios.get(`http://${endpoint}:3030/api/books`).then((res) => {
      const sortBooks = res.data.sort((a: Book, b: Book) => new Date(b.endDate).getTime() - new Date(a.startDate).getTime());
      setBooks(sortBooks);
  ...
  const handleSearch = (query: string) => {
    ...
    const filtered = books.filter((book: Book) => {
      ...
      return title.includes(lowerQuery) || author.includes(lowerQuery) || genre.includes(lowerQuery);
    });
